...
// ==========================================
// Mobile Floating Menus UX (Step 4)
// - Smooth open/close animations
// - Body scroll lock
// - ESC to close
// ==========================================

const MOBILE_MENU_IDS = [
  "fab-menu-container",
...
function _openEl(el, displayClass){
  if (!el) return;
  el.classList.remove("d-none");
  if (displayClass) el.classList.add(displayClass);
  _nextFrame(() => el.classList.add("is-open"));
}

function _closeEl(el, displayClass){
  if (!el) return;
...
function _anyMenuOpen(){
  return MOBILE_MENU_IDS.some(id => document.getElementById(id)?.classList.contains("is-open"));
}
...
function _syncOverlay(){
  ...
  if (!overlay) return;
  if (_anyMenuOpen()){
    _openEl(overlay, null);
    document.body.classList.add("no-scroll");
  } else {
    _closeEl(overlay, null);
